#!/bin/bash
sudo yum -y update
sudo yum -y install httpd
sudo yum install -y mysql
sudo amazon-linux-extras enable php7.4
sudo yum clean metadata
sudo yum install -y php php-{pear,cgi,common,curl,mbstring,gd,mysqlnd,gettext,bcmath,json,xml,fpm,intl,zip,imap,devel}

sudo usermod -a -G apache ec2-user
sudo chown -R ec2-user:apache /var/www
sudo chmod 2775 /var/www
sudo find /var/www -type d -exec chmod 2775 {} \;
sudofind /var/www -type f -exec chmod 0664 {} \;
#echo "<?php phpinfo(); ?>" > /var/www/html/index.php


sudo cat <<EOF > /var/www/html/dbinfo.inc
<?php

define('DB_SERVER', 'appmariadb.cqpiwui8z0cu.us-east-1.rds.amazonaws.com');
define('DB_USERNAME', 'webapp');
define('DB_PASSWORD', 'Pass-word01');
define('DB_DATABASE', 'appmariadb');

?>
EOF

cd /var/www/html
wget https://raw.githubusercontent.com/gagasgit/Terraform_High_Available_APP/main/index.php

sudo service httpd start
chkconfig httpd on

# script is token from official aws documetation example
# https://docs.aws.amazon.com/AmazonRDS/latest/UserGuide/CHAP_Tutorials.WebServerDB.CreateWebServer.html#CHAP_Tutorials.WebServerDB.CreateWebServer.Apache